...
function makebutton(jsonObj) {
  for (var i = 0; i < jsonObj.length; i++) {
    document
      .getElementById("차례제목")
      .insertAdjacentHTML(
        "beforeend",'<div class="버튼"><button class="차례버튼" id="버튼' + i + '" onclick="request(\'main_0_story/' + jsonObj[i][0] + "'," + 2 + "," + i + ')">' + jsonObj[i][1] + "</button></div>");
  }
  ...
  for (var i = 0; i < jsonObj["storyList"].length; i++) {
    ...
    if (jsonObj["storyList"][i]["prop"] == "Image") {
      ...
      for (let j = 0; j < optlist.length; j++) {
        let linenum = jsonObj["storyList"][i]["targetLine"]["option" + (j + 1)];
