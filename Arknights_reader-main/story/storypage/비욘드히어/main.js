...
function makebutton(jsonObj) {
  for (var i = 0; i < jsonObj.length; i++) {
    ...
    let path = '../../resource/storyjson/activities/비욘드히어/' + jsonObj[i];
...
function readname(jsonObj, i, path) {
  document.getElementById("자리"+i).insertAdjacentHTML("beforeend",'<button class="차례버튼" id="버튼' + i + '" onclick="request(\''+ path + "'," + 2 + "," + i + ')">' + jsonObj["storyCode"]+" "+jsonObj["storyName"] + "</button>");
